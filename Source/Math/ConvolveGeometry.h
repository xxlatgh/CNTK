...
public:
    ...
    int GetLowerPad(size_t dim) const
    {
        ...
        int center = extra / 2;
        int dilSize = (int)m_dilation[m_dilation.size() == 1 ? 0 : dim];

        int pad = (kernSize - 1)/2 - center;

        if(((outSize - 1) * stride + kernSize) > (2 * pad + inpSize)) RuntimeError("When (inputSize-1) is divisible by stride, even filter size is not supported by cudnn convolution Engine.\n");
